...
void submenuBusqueda(Libro libros[], Usuario usuarios[], Bibliotecario bibliotecarios[]) {
	...
	do {
		...
		case 'B': {
			...
			transform(apellido.begin(), apellido.end(), apellido.begin(), ::tolower); // Convertir a minúsculas
			// Lógica para buscar usuario por nombre y apellido
			bool encontrado = false;
			for (int i = 0; i < NUM_USUARIOS; ++i) {
				string nombreUsuario = usuarios[i].nombre;
				string apellidoUsuario = usuarios[i].apellido;
				transform(nombreUsuario.begin(), nombreUsuario.end(), nombreUsuario.begin(), ::tolower); // Convertir a minúsculas
				transform(apellidoUsuario.begin(), apellidoUsuario.end(), apellidoUsuario.begin(), ::tolower); // Convertir a minúsculas
			...
			if (!encontrado) {
				cout << "No se encontró ningún usuario con ese nombre y apellido.\n";
			}
			break;
		}
		case 'C': {
			...
			transform(nombre.begin(), nombre.end(), nombre.begin(), ::tolower); // Convertir a minúsculas
			transform(apellido.begin(), apellido.end(), apellido.begin(), ::tolower); // Convertir a minúsculas
			// Lógica para buscar bibliotecario por nombre y apellido
			bool encontrado = false;
			for (int i = 0; i < NUM_BIBLIOTECARIOS; ++i) {
				string nombreBibliotecario = bibliotecarios[i].nombre;
				string apellidoBibliotecario = bibliotecarios[i].apellido;
				transform(nombreBibliotecario.begin(), nombreBibliotecario.end(), nombreBibliotecario.begin(), ::tolower); // Convertir a minúsculas
				transform(apellidoBibliotecario.begin(), apellidoBibliotecario.end(), apellidoBibliotecario.begin(), ::tolower); // Convertir a minúsculas
				if (nombreBibliotecario == nombre && apellidoBibliotecario == apellido) {
			...
			if (!encontrado) {
				cout << "No se encontró ningún bibliotecario con ese nombre y apellido.\n";
			}
			break;
		}
		case 'D':
...
int main() {
	...
	string listaLibros[] = {"Aprende C", "Aprende PHP", "Aprende Linux", "Aprende C++", "Aprende Ubuntu", "Clean Code", "The Art of Computer Programming", "Programming Pearls", "Introduction to Algorithms"};
	...
	// Generar datos aleatorios utilizando sobrecarga de funciones
